 ...
 * Open contests with available slots appear first; full contests are grouped
 * in a collapsible "FULL" section. Clicking an open contest navigates to the
 * Lineup Builder with that contest pre-selected.
 */
...
export default function Contests() {
  ...
  // Split contests into available and full groups
  const { available, full } = useMemo(() => {
    ...
    // Sort available: LIVE first, then OPEN, then by spots remaining (ascending)
    avail.sort((a, b) => {
      ...
      // Within same status, sort by spots remaining (fewest first = most urgent)
      const spotsA = getSpotsLeft(a);
      const spotsB = getSpotsLeft(b);
      return spotsA - spotsB;
    });

    return { available: avail, full: fullList };
  }, [data?.contests]);
          ...
          <p className="text-muted-foreground text-xs sm:text-sm mt-1">
            ...
            {available.length > 0 && (
              <span className="text-teal ml-1">
                ({available.length} with open slots)
              </span>
            )}
          </p>
        </div>
